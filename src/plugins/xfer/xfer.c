          ...
          socklen_t addrlen, int port, int sock, const char *local_filename)
{
    ...
    if (XFER_IS_RECV(type))
    {
        ...
        new_xfer->local_addrlen = sizeof(struct sockaddr_storage);
        new_xfer->local_address_str = strdup("");
        new_xfer->remote_address = calloc(1, sizeof(struct sockaddr_storage));
        new_xfer->remote_addrlen = sizeof(struct sockaddr_storage);
...
/*
 * Safely sets the remote address field.
 */

void
xfer_set_remote_address (struct t_xfer *xfer,
                         ...
                         char *addr_str)
{
    ...
    memcpy(xfer->remote_address, addr, length);

    if (xfer->remote_address_str)
        free(xfer->remote_address_str);

    if (addr_str)
        xfer->remote_address_str = strdup(addr_str);
    else
        xfer->remote_address_str = strdup("");
}
